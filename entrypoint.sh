...
java \
  ...
  -Dcolumns=${INPUT_COLUMNS} \
  -Dcode=${INPUT_CODE} \
  -Dconf=${INPUT_CONF} \
  -Dbaseline=${INPUT_BASELINE} \
  -Dcustom_detectors_dir=${INPUT_CUSTOM_DETECTORS_DIR} \
  -Ddetectors=${INPUT_DETECTORS} \
  -Dskip_detectors=${INPUT_SKIP_DETECTORS} \
  -Dsbom=${INPUT_SBOM} \
  -Dsbom_format=${INPUT_SBOM_FORMAT} \
  -Dsecrets_mode=${INPUT_SECRETS_MODE} \
  -Dstandard=${INPUT_STANDARD} \
  -Dfail_on=${INPUT_FAIL_ON} \
  -Dtry_all_scans=${INPUT_TRY_ALL_SCANS} \
